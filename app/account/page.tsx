...
function getPlanLabel(plan?: string | null, status?: string | null) {
  if (plan === 'vip' && ['active', 'trialing', 'past_due'].includes(status || '')) {
    return 'VIP';
  }
...
function getPlanDescription(planLabel: string, status?: string | null) {
  if (planLabel === 'VIP') {
    return `Status: ${status || 'active'}`;
  }

  return status ? `Status: ${status}` : 'No subscription synced yet';
}
...
export default async function AccountPage() {
  ...
  const currentPlan = getPlanLabel(subscription?.plan, subscription?.status);
  const currentPlanDescription = getPlanDescription(currentPlan, subscription?.status);
  const defaultPet =
    ...
    <>
      ...
      <main className='container-shell py-10'>
        ...
        <p className='page-subtitle mx-0'>
          This is your EchoPaws account and membership hub. You can view your
          login status, manage your Stripe subscription, check your pets and
          default chat target, and securely sign out.
        </p>
        ...
        <section className='mt-8 grid gap-5 md:grid-cols-[1.1fr_.9fr]'>
          <div className='glass-card p-6'>
            ...
            <div className='mt-5 grid gap-4 md:grid-cols-3'>
              <div className='rounded-2xl border border-black/5 bg-white p-4'>
                ...
                <div className='mt-2 rounded-2xl bg-slate-50 px-3 py-3'>
                  ...
                  <div className='mt-1 break-all text-xs leading-6 text-muted'>
                    {fullEmail}
                  </div>
                </div>
              </div>
            ...
            <div className='mt-5 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-900'>
              Plan status is synced from Stripe via webhook. If you just upgraded
              to VIP, allow a few moments for the webhook to update your
              subscription record.
            </div>
          </div>
